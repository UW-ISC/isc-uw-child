...
if ( ! function_exists( 'isc_user_guide_menu' ) ) :
	...
	function isc_user_guide_menu( $show_feedback = false ) {
		...
		$last_li = $show_feedback ? '<li class="nav-item has-children user-guide-feedback-menu-item"><button class="nav-link first collapsed" data-toggle="collapse"  data-target="#feedback" aria-controls="#feedback" aria-expaded="false" title="Link to feedback form"> Feedback? <i class="fa fa-2x accordion-handle"></i> </button> <ul class="children depth-1 collapse" id="feedback" data-parent="#tocAccordion"> '.do_shortcode('[contact-form-7 title="User Guide Feedback"]').' </ul> </li>' : '';
